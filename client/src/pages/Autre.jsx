...
export default function Autre() {
 ...
 useEffect(() => {
   async function getRecords() {
     const response = await fetch(`http://localhost:5050/relation_autre`);
...
function EditToolbar(props) {
  ...
  return (
    <GridToolbarContainer>
      ...
      <Button color="primary" startIcon={<AddIcon />}   onClick={() => navigate("/create-autre")}>
          Ajouter 
      </Button>
  ...
  const handleRowEditStop = (params, event) => {
   ...
   const handleDeleteClick = (id) => () => {
    async function deleteRow(){
      ...
      await fetch(`http://localhost:5050/relation_autre/${id}`, {
      method: "DELETE"
    });
